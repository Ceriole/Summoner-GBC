...
void main(void)
{
	...
	while(1)
	{
		obj_update(&fire);
		fire.x += 8;
		fire.y = OBJ_SCRN_TO_POS(SCREENHEIGHT / 2) + 32 - SIN_TABLE[((fire.x >> 1) % 128)];
		uint8_t dexxy = 0;
